...
const AddRecipe = () => {
    ...
    const [recipe_time, setRecipeTime] = useState('0')
    ...
    const choosePhoto = () => {
      ...
      const addRecipe = async () =>{
        ...
        data.append('user_id', parse.id)


        axios.post(baseUrl+'/add_recipe', data, {
            headers: { "Content-type": "multipart/form-data" }
        }).then((response) => {
            if(response.data.msg == 'Added Successfully'){
    ...
    return(
        <TouchableWithoutFeedback onPress={() => Keyboard.dismiss()}>
            ...
            <TouchableOpacity onPress={(() => addRecipe())} style={styles.SignUpButton}>
